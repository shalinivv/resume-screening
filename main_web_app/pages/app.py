...
st.markdown(f'<h1 style="color:#33ff33;font-family:Copper Black;font-size:40px;">{"Resume Screening"}</h1>', unsafe_allow_html=True)
...
if len(Jobs['Name']) > 1:
    ...
    if option_yn == 'YES':
        ...
        fig = go.Figure(data=[go.Table(header=dict(values=["Job No.", "Job Desc. Name"], line_color='darkslategray',fill_color='lightskyblue'),
                                       cells=dict(values=[index, Jobs['Name']], line_color='darkslategray',fill_color='black'))])
...
if option_yn == 'YES':
    st.markdown("---")
    st.markdown(f'<h1 style="color:#33ff33;font-family:Copper Black;font-size:35px;">{"Job Description"}</h1>', unsafe_allow_html=True)
    fig = go.Figure(data=[go.Table(
        header=dict(values=["Job Description"],fill_color='#f0a500',align='center', font=dict(color='black', size=16)),
        cells=dict(values=[Jobs['Context'][index]],fill_color='#f4f4f4',align='left'))])
...
fig1 = go.Figure(data=[go.Table(
    header=dict(values=["Rank", "Name", "Scores"],fill_color='#00416d',align='center', font=dict(color='black', size=16)),
    cells=dict(values=[Ranked_resumes.Rank, Ranked_resumes.Name, Ranked_resumes.Scores],fill_color='#000000',align='left'))])
...
st.markdown(f'<h1 style="color:#33ff33;font-family:Copper Black;font-size:35px;">{"Topics and Topic Related Keywords"}</h1>', unsafe_allow_html=True)
...
st.markdown(f'<h1 style="color:#33ff33;font-family:Copper Black;font-size:35px;">{"Topic Modeling with Resume Using LDA "}</h1>', unsafe_allow_html=True)
...
fig3 = px.sunburst(df, path=['Dominant Topic', 'Names'], values='Topic % Contribution',color='Dominant Topic', color_continuous_scale='viridis', width=800, height=800, title="Topic Distribution Graph")
st.write(fig3)
...
if option_2 == 'YES':
    ...
    st.markdown("---")
    st.markdown(f'<h1 style="color:#33ff33;font-family:Copper Black;font-size:35px;">{"Resume"}</h1>', unsafe_allow_html=True)
    ...
    fig = go.Figure(data=[go.Table(
        header=dict(values=["Resume"],fill_color='#000000',
                    align='center', font=dict(color='black', size=16)),
        cells=dict(values=[str(value)],fill_color='#f4f4f4',align='left'))])
